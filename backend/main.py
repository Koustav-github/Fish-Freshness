...
YOLO_CONF_THRESHOLD  = 0.20   # minimum score for YOLO to report a detection at all

# ── Image guardrails ──────────────────────────────────────────────────────────
MAX_IMAGE_DIM        = 4096   # px — above this rembg / YOLO risk OOM
...
def _warmup_models() -> None:
    """
    Run a single dummy inference through both models immediately after loading.
    This forces CUDA kernel compilation and TF graph tracing so the very first
    real request does not pay the cold-start penalty.
    """
    print("🔥 Warming up YOLO…")
    dummy_img = np.zeros((640, 640, 3), dtype=np.uint8)
    yolo_model.predict(source=dummy_img, imgsz=640, conf=0.20, verbose=False)

    print("🔥 Warming up EfficientNetV2S…")
    dummy_tensor = np.zeros((1, IMG_SIZE, IMG_SIZE, 3), dtype=np.float32)
    classifier.predict(dummy_tensor, verbose=0)

    print("✅ Warm-up complete — first request will be fast.")
...
def validate_image(b64_raw: str, image: np.ndarray) -> None:
    """
    Enforce payload size and dimension limits before any heavy compute runs.

    Checks (in order):
      1. Base64 payload ≤ 5 MB  — prevents network / memory abuse
      2. Both dimensions ≥ 128 px — prevents U²-Net segmentation failure on
         tiny images where there are not enough pixels to form a clean mask
      3. Both dimensions ≤ 4096 px — prevents OOM crashes in rembg / YOLO
         on extremely high-resolution inputs
    """
    # 1. Payload size
    if len(b64_raw.encode()) > MAX_B64_BYTES:
        raise HTTPException(
            status_code=413,
            detail=f"Payload too large — maximum allowed is 5 MB. Received {len(b64_raw.encode()) / 1024 / 1024:.1f} MB.",
        )
...
def yolo_fish_present(image: np.ndarray) -> tuple[bool, float]:
    """
    Run YOLOv8 on the image purely as a presence gate.

    Returns (fish_found: bool, max_conf: float).

    Returns True if YOLO detects at least one object with conf >= YOLO_CONF_THRESHOLD.
    Does NOT crop or use the bounding box coordinates downstream.
    """
...
def predict(request: Request, req: ImageRequest):
    ...
    # 2. Guardrails — payload size + dimension bounds
    validate_image(req.image_base64, image)

    original_b64 = cv2_to_b64(image)
    H, W = image.shape[:2]

    # 3. YOLO presence gate — terminate early if no fish found
    fish_found, _ = yolo_fish_present(image)
    if not fish_found:
        raise HTTPException(status_code=422, detail="No fish detected in the image. Please upload a clear photo of a fish.")

    # 4. Segment fish on the full image via rembg (no cropping)
